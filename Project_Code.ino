...
#include <TimeLib.h> // include Timer library code
...
int hour_reader; //Declare houer reader
...
void setup()
{
  ...
  mlx.begin();//Inicializa o MLX90614
   ...
   Serial1.begin(9600);// initialize serial for timer conver use "T" and The current Unix epoch time 
...
void loop()
{
  ...
  if(numOfpeopel < MAX_Peopel)//check if there is place to enter
  {
    ...
    if(digitalRead(FRINT_IRSensor)!= 1) //Check the front IR senser if it LOW then take the temp
    {
      ...
      {
        numOfpeopel++; // cal the number of peopel inside
        total_pepelenter++; // cal the total peopel has enter
      ...
      {
        total_pepelReject++;//cal the total peopel been reject
        lcd.clear();
        lcd.setCursor(0, 0);
  ...
  for(int i = 9 ; i < 17 ; i++)
  {
    if(hour_reader == i)
    {
      //cal the peopel been reject per hour
      reject_peopel[hour_reader - 9] = total_pepelReject - reject_peopel[0]-reject_peopel[1]-reject_peopel[2]-reject_peopel[3]-reject_peopel[4]-reject_peopel[5]-reject_peopel[6]-reject_peopel[7];
      // cal the number of people been enter per hour
      enter_peopel[hour_reader - 9] = total_pepelenter - enter_peopel[0]-enter_peopel[1]-enter_peopel[2]-enter_peopel[3]-enter_peopel[4]-enter_peopel[5]-enter_peopel[6]-enter_peopel[7];
    }//end of if
  }//end of FOR
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.print("['Year', 'enter', 'reject'],");
          client.print("['09:00',");
          ...
          client.print("],");
          client.print("['17:00',  0,      0]");
          client.print("]);");
          client.print("var options = {");
          client.print("title: 'Access control',");
